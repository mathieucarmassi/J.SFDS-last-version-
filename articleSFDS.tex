...
On the two first rows the two first models (without and with surrogate) which have only these four parameters to estimate.
...
Figure \ref{fig:calibrationSeq} illustrates the new results based on the Gaussian process build after the sequential design and proves that with a better surrogate, calibration is consistent with physics and with the models $\mathcal{M}_1$ and $\mathcal{M}_3$.\newline
...
To compare the prediction ability of the four models, a cross validation (CV) is performed. Three days of data (chosen randomly) are taken off the calibration dataset for each of the $100$ repetitions of the CV. 
...
However for $\mathcal{M}_2$ and $\mathcal{M}_4$ the coverage rates are below this level. As expected the coverage rates of $\mathcal{M}_2'$ and $\mathcal{M}_4'$ increase since the quality of the surrogate has been improved. \newline


Overall, the model $\mathcal{M}_3$ has better results than the others. This conclusion can be explained twofold. First, the code realizes a better prediction than the surrogate. Second, a correlation structure remains in the error. Adding the discrepancy in the model allows to catch up the real results. In the case where a surrogate is used instead of the code, the CV produces worse results which was expected since the number of points chosen in the DOE is not sufficient to reproduce the behavior of the code. The use of the sequential design to add only $10$ points had allowed to improve drastically the models for calibration and in prediction.\newline
...
In most of industrial cases, the code is time consuming. The recourse of the models $\mathcal{M}_2$ and $\mathcal{M}_4$ are often unavoidable. The quality of the Gaussian process used to emulate the code has a real impact on calibration accuracy. As a limited number of code calls is authorized, methods such as sequential design \citep{damblin2018} can be useful to improve the quality of the Gaussian process with a restricted number of additional points. \newline
